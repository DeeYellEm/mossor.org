 ...
 */
 class spacious_featured_single_page_widget extends WP_Widget {
 	function spacious_featured_single_page_widget() {
		...
		parent::WP_Widget( false, $name= __( 'TG: Featured Single Page', 'spacious' ), $widget_ops, $control_ops);
 	}
	...
	function widget( $args, $instance ) {
 		...
 		if( $page_id ) {
			...
			$output .= '<a class="read-more" href="'. get_permalink() .'">'. of_get_option( 'spacious_read_more_text', __( 'Read more', 'spacious' ) ) .'</a>';
	 		$output .= $after_widget;
	 		endwhile;
	 		// Reset Post Data
	 		wp_reset_postdata();
...
class spacious_service_widget extends WP_Widget {
	...
	?>
		<p>
			<input class="checkbox" type="checkbox" <?php echo $image_link; ?> id="<?php echo $this->get_field_id('image_link'); ?>" name="<?php echo $this->get_field_name('image_link'); ?>" /> <label for="<?php echo $this->get_field_id('image_link'); ?>"><?php _e( 'Link featured image to their respective page', 'spacious' ); ?></label>
		</p>
		<?php for( $i=0; $i<6; $i++) { ?>
			<p>
 			...
 			while( $get_featured_pages->have_posts() ):$get_featured_pages->the_post();
				...
				<div class="<?php echo $service_class; ?>">
					...
					<div class="more-link-wrap">
						<a class="more-link" title="<?php the_title_attribute(); ?>" href="<?php the_permalink(); ?>"><?php echo of_get_option( 'spacious_read_more_text', __( 'Read more', 'spacious' ) ); ?></a>	
					</div>					
				</div>
				<?php $j++; ?>					
			<?php endwhile;
...
class spacious_call_to_action_widget extends WP_Widget {
 	...
 	function form( $instance ) {
		...
		<p>
			<input class="checkbox" type="checkbox" <?php echo $new_tab; ?> id="<?php echo $this->get_field_id('new_tab'); ?>" name="<?php echo $this->get_field_name('new_tab'); ?>" /> <label for="<?php echo $this->get_field_id('new_tab'); ?>"><?php _e( 'Open in new tab', 'spacious' ); ?></label>
		</p>
		<?php
	}
		...
		?>
			<div class="call-to-action-content-wrapper clearfix">
				...
				?>	
					...
					?>				
					<a class="call-to-action-button" <?php if( $new_tab == 'true' ) { echo 'target="_blank"'; } ?> href="<?php echo $button_url; ?>" title="<?php echo esc_attr( $button_text ); ?>"><?php echo esc_html( $button_text ); ?></a>
				<?php
				}
				?>
			</div>
...
class spacious_testimonial_widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		<div class="testimonial-post"><?php 
													// For WPML plugin compatibility
													if ( function_exists( 'icl_t' ) ) {
														$text = icl_t( 'Spacious Pro', 'TG: Testimonial widget text' . $this->id, $text );
													}
		...
		<div class="testimonial-author">
			<span><?php 
						// For WPML plugin compatibility
						if ( function_exists( 'icl_t' ) ) {
							$name = icl_t( 'Spacious Pro', 'TG: Testimonial widget name' . $this->id, $name );
						}
						echo esc_html( $name ); ?></span>
			<?php 
				// For WPML plugin compatibility
				if ( function_exists( 'icl_t' ) ) {
					$byline = icl_t( 'Spacious Pro', 'TG: Testimonial widget byline' . $this->id, $byline );
				}
...
class spacious_featured_posts_widget extends WP_Widget {
	...
	function spacious_featured_posts_widget() {
		$widget_ops = array( 'classname' => 'widget_featured_posts', 'description' => __( 'Display latest posts or posts of specific category', 'spacious') );
		$control_ops = array( 'width' => 200, 'height' =>250 );
		parent::WP_Widget( false,$name= __( 'TG: Featured Posts', 'spacious' ),$widget_ops);
	}
	...
	function form( $instance ) {
		...
		<p><input type="radio" <?php checked($image_size, 'featured') ?> id="<?php echo $this->get_field_id( 'image_size' ); ?>" name="<?php echo $this->get_field_name( 'image_size' ); ?>" value="featured"/><?php _e( 'Large Featured Image', 'spacious' );?><br />  
		 <input type="radio" <?php checked($image_size,'featured-blog-medium') ?> id="<?php echo $this->get_field_id( 'image_size' ); ?>" name="<?php echo $this->get_field_name( 'image_size' ); ?>" value="featured-blog-medium"/><?php _e( 'Medium Featured Image', 'spacious' );?><br /></p>

		<p><input type="radio" <?php checked($type, 'latest') ?> id="<?php echo $this->get_field_id( 'type' ); ?>" name="<?php echo $this->get_field_name( 'type' ); ?>" value="latest"/><?php _e( 'Show latest Posts', 'spacious' );?><br />  
		 <input type="radio" <?php checked($type,'category') ?> id="<?php echo $this->get_field_id( 'type' ); ?>" name="<?php echo $this->get_field_name( 'type' ); ?>" value="category"/><?php _e( 'Show posts from a category', 'spacious' );?><br /></p>

		<p>
			<label for="<?php echo $this->get_field_id( 'category' ); ?>"><?php _e( 'Select category', 'spacious' ); ?>:</label>
	...
	function widget( $args, $instance ) {
		...
		?>
		<?php
			if( $image_size == 'featured-blog-medium' ) { $featured = 'featured-blog-medium'; $image_class = 'post-featured-image'; }
			else { $featured = 'featured'; $image_class = 'post-featured-iamge-large'; }
...
class spacious_our_clients_widget extends WP_Widget {
	...
	function spacious_our_clients_widget() {
		$widget_ops = array( 'classname' => 'widget_our_clients', 'description' => __( 'Widget to show clients/partners images', 'spacious') );
		$control_ops = array('width' => 200, 'height' => 250);
		parent::WP_Widget( false, $name= __( 'TG: Our Clients', 'spacious' ), $widget_ops, $control_ops );
	}

	function form( $instance ) {		
		$instance = wp_parse_args( (array) $instance, array( 'title' => '', 'text' => '', 'image-1' => '', 'image-2' => '', 'image-3' => '', 'image-4' => '', 'image-5' => '', 'link-1' => '', 'link-2' => '', 'link-3' => '', 'link-4' => '', 'link-5' => '', 'hover-text1' => '', 'hover-text2' => '', 'hover-text3' => '', 'hover-text4' => '', 'hover-text5' => '', ) );	
		$title = esc_attr($instance['title']);
		$text = esc_textarea( $instance[ 'text' ] );	 
		for ( $i=1; $i<6; $i++ ) {
 			$image = 'image-'.$i;
	...
	?>
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id($image); ?>"><?php _e( 'Image Path Link ', 'spacious' ); echo $i; ?></label> 
		...
		<p><?php _e('Redirect Link ', 'spacious'); echo $i;?>
		...
		<p>
			<label for="<?php echo $this->get_field_id($hover_text); ?>"><?php _e( 'Hover Title Text ', 'spacious' ); echo $i; ?></label> 
	...
	function widget( $args, $instance ) {
		...
		for( $i=1; $i<6; $i++ ) {
 			...
 			if( !empty( $link ) ) array_push( $link_array, $link );
 			if( !empty( $hover_text ) ) array_push( $hover_text_array, $hover_text );
 		}
			...
			for( $i=1; $i<6; $i++ ) {
				...
				if( !empty( $image_array[$j] ) ) {
					...
					if( !empty( $link_array[$j] ) ) {
						...
						$output .= '<img src="'.$image_array[$j].'" alt="'.$hover_text_array[$i].'">';
					}									
					$output .=	'</div>';
				}
			}
